...
export function BabyInfoCard({ summary }: { summary: DashboardSummary }) {
  return (
    <Card className="flex items-center gap-4">
      <Avatar label={summary.baby.name} role="baby" size="lg" />
      <div className="min-w-0 flex-1">
        <p className="text-lg font-semibold text-soft-charcoal">{summary.baby.name}</p>
        <p className="mt-1 text-sm text-dark-gray">{summary.baby.age_display}</p>
